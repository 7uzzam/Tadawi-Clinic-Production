...
const phaseDir = path.join(root, 'docs', 'integration-v2-5-3');
const tracePath = path.join(phaseDir, 'REQUIREMENTS-TRACEABILITY.md');
const evidenceDir = path.join(phaseDir, 'evidence');
const REQUIRED_IDS = [
  ...Array.from({ length: 16 }, (_, i) => `OWN-253-${String(i + 1).padStart(3, '0')}`),
  ...Array.from({ length: 10 }, (_, i) => `ID-253-${String(i + 1).padStart(3, '0')}`),
  ...Array.from({ length: 18 }, (_, i) => `LIC-253-${String(i + 1).padStart(3, '0')}`),
  'UAT-253-001', 'UAT-253-002', 'REG-253-001', 'REL-253-001',
];
...
for (const line of text.split('\n')) {
  if (!/^\|\s*[A-Z]+-253-\d+\s*\|/.test(line)) continue;
  const cells = line.split('|').map((c) => c.trim()).filter((_, i, arr) => i > 0 && i < arr.length - 1);
  if (cells.length < 6) continue;
  const id = cells[0];
  if (!/^(OWN|ID|LIC|UAT|REG|REL)-253-\d+$/.test(id)) continue;
...
console.log(`Parsed ${rows.length} V2-5.3 requirements`);
if (errors.length) { console.error('V2-5.3 RELEASE GATE FAIL'); errors.forEach((e) => console.error(' - ' + e)); process.exit(1); }
console.log('V2-5.3 RELEASE GATE PASS');
