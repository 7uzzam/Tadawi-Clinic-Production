...
const phaseDir = path.join(root, 'docs', 'integration-v2-5-4');
const tracePath = path.join(phaseDir, 'REQUIREMENTS-TRACEABILITY.md');
const evidenceDir = path.join(phaseDir, 'evidence');
const REQUIRED_IDS = [
  ...Array.from({ length: 45 }, (_, i) => `RBAC-254-${String(i + 1).padStart(3, '0')}`),
  'UAT-254-001', 'UAT-254-002', 'REG-254-001', 'REL-254-001',
];
...
for (const line of text.split('\n')) {
  if (!/^\|\s*[A-Z]+-254-\d+\s*\|/.test(line)) continue;
  const cells = line.split('|').map((c) => c.trim()).filter((_, i, arr) => i > 0 && i < arr.length - 1);
  if (cells.length < 6) continue;
  const id = cells[0];
  if (!/^(RBAC|UAT|REG|REL)-254-\d+$/.test(id)) continue;
...
console.log(`Parsed ${rows.length} V2-5.4 requirements`);
if (errors.length) { console.error('V2-5.4 RELEASE GATE FAIL'); errors.forEach((e) => console.error(' - ' + e)); process.exit(1); }
console.log('V2-5.4 RELEASE GATE PASS');
