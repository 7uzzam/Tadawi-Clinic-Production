...
const REQUIRED_IDS = [
  ...Array.from({ length: 13 }, (_, i) => `BUILD-257-${String(i + 1).padStart(3, '0')}`),
  ...Array.from({ length: 13 }, (_, i) => `LIFE-257-${String(i + 1).padStart(3, '0')}`),
  ...Array.from({ length: 10 }, (_, i) => `MIG-257-${String(i + 1).padStart(3, '0')}`),
  ...Array.from({ length: 4 }, (_, i) => `COMP-257-${String(i + 1).padStart(3, '0')}`),
  ...Array.from({ length: 20 }, (_, i) => `REL-257-${String(i + 1).padStart(3, '0')}`),
];
...
for (const line of text.split('\n')) {
  if (!/^\|\s*[A-Z]+-257-\d+\s*\|/.test(line)) continue;
  const cells = line.split('|').map((c) => c.trim()).filter((_, i, arr) => i > 0 && i < arr.length - 1);
  if (cells.length < 6) continue;
  const id = cells[0];
  if (!/^(BUILD|LIFE|MIG|COMP|REL)-257-\d+$/.test(id)) continue;
...
console.log(`Parsed ${rows.length} V2-5.7 requirements`);
if (errors.length) { console.error('V2-5.7 RELEASE GATE FAIL'); errors.forEach((e) => console.error(' - ' + e)); process.exit(1); }
console.log('V2-5.7 RELEASE GATE PASS');
