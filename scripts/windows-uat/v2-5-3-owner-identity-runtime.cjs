...
function main() {
  ...
  const scenarios = run('scripts/v2-5-3-scenarios-all.cjs');
  const unit = run('tests/baseline/test-v2-5-3-owner-identity-license.js');
  const ownerRbac = run('scripts/windows-uat/owner-rbac-runtime.cjs');
  ...
  const ok = scenarios.status === 0 && unit.status === 0 && ownerRbac.status === 0;
  const deviceA = {
    device: 'A',
    role: 'owner-identity-uat',
    startedAt,
    ...
    ownerRbacExit: ownerRbac.status,
    build,
    result: ok ? 'PASS' : 'FAIL',
  };
  const deviceB = {
    device: 'B',
    role: 'race-and-transfer-uat',
    startedAt,
    finishedAt: new Date().toISOString(),
    host: { platform: process.platform, arch: process.arch, release: os.release() },
    scenarios: ['O02-two-device-race', 'I02-device-transfer-sync-block'],
  ...
  fs.writeFileSync(path.join(evidenceDir, 'failure-recovery.json'), `${JSON.stringify({
    ...
    scenarios: ['O02-two-device-race', 'O03-emergency-recovery-audit', 'L01-limits-grace-upgrade-downgrade'],
    result: scenarios.status === 0 ? 'PASS' : 'FAIL',
  }, null, 2)}\n`);
