...
function run(rel) {
  ...
  return { status: r.status, stdout: (r.stdout || '').slice(-500), stderr: (r.stderr || '').slice(-500) };
}
...
function main() {
  ...
  const scenarios = run('scripts/v2-5-2-scenarios-all.cjs');
  const unit = run('tests/backup/backup-sync-harden-v2.test.js');
  ...
  const deviceB = {
    device: 'B',
    role: 'peer-roundtrip-offline-uat',
    startedAt,
    finishedAt: new Date().toISOString(),
    host: { platform: process.platform, arch: process.arch, release: os.release() },
    scenarios: ['S01-ab-roundtrip', 'S02-device-c-catchup', 'S03-offline-queue-reconnect'],
  ...
  fs.writeFileSync(path.join(evidenceDir, 'failure-recovery.json'), `${JSON.stringify({
    ...
    scenarios: ['B02-cloud-upload-and-quota', 'B05-upload-resume', 'S03-offline-queue-reconnect'],
    result: scenarios.status === 0 ? 'PASS' : 'FAIL',
  }, null, 2)}\n`);
